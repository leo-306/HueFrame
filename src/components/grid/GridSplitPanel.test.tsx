...
describe('GridSplitPanel', () => {
  ...
  it('shows the grid size picker and export button after a photo is uploaded', async () => {
    ...
    expect(screen.getByRole('heading', { name: '单元格边框' })).toBeInTheDocument()
    ...
    expect(screen.getByRole('slider', { name: '背景透明度' })).toHaveAttribute('aria-valuemax', '100')
    expect(screen.getByLabelText('背景颜色')).toHaveValue('#f9f9f8')
    expect(screen.getByText('100%')).toBeInTheDocument()
    expect(screen.getByRole('slider', { name: '边框粗细' })).toHaveAttribute('aria-valuemax', '12')
    expect(screen.getByLabelText('边框颜色')).toHaveValue('#d8d8d7')
